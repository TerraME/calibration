...
discreteCostanzaMultiLevel = function(cs1, cs2, attribute)
	...
	fitnessSum = pixelByPixelString(cs1,cs2,attribute,attribute) -- fitnessSum is the Sum of all the fitness from each square ixi , it is being initialized as the fitnisess of the 1x1 square
	x = cs1:getXDim()
	y = x
	local squareBySquare = function(dim) -- function that returns the fitness of a particular dimxdim square.
	...
	for i=2,x do -- increase the square size and calculate fitness for each square.
		fitnessSum = fitnessSum + squareBySquare(i)
	end
